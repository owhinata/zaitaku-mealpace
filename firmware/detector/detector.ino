...
static void prof_add(uint32_t total, uint32_t wall, const HopResult* r, uint32_t send, bool ok, uint32_t wait_ms) {
  if (prof_n < PROF_N) {
...
void loop() {
  ...
  prof_add(t_end - t_take, wall_ms * 1000u, &r, ok ? t_end - t_send : 0, ok, wait_ms);
#endif
}
