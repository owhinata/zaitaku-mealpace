// 移植の答え合わせ（PC 側、#17・#21）。synth / fft512 / audio_features / imu_features をリンクし、
// 合成信号（seed 固定）で計算して標準出力に出す。check_port.py が analysis/features.py（AF_PROFILE 0）または
// analysis/features_m2.py（AF_PROFILE 1）と比べる。
...
//   M2: 上に -DAF_PROFILE=1 を足して build-host/check_port_m2 にし、check_port.py --profile m2 で比べる。
//
// 出力（1 行 1 レコード、値は %.9g）:
//   profile <AF_PROFILE> <AF_FRAME_HOP>   ビルドの式（check_port.py が --profile と照合する）
//   full_audio <15 値>            全窓版（3.0 秒の合成音声（16 kHz）の先頭 1.0 秒。M2 は間引き後の 8000 サンプル）
//   full_imu <14 値>              IMU 14 次元（t_ms < 1000 の行）
//   reuse <k> <15 値>             再利用版（16 kHz のスライス 4000 を順に入れる）。k = 0 は最初に窓が満ちたとき、k = 1..8 はその後の 8 ホップ
#include <stdio.h>
#include <stdint.h>
#include "synth.h"
...
static const uint32_t AUDIO_TOTAL = AUDIO_SECONDS * AF_IN_HZ;              // 48000（16 kHz）
static const uint32_t AUDIO_TOTAL_AF = AUDIO_TOTAL / AF_DECIMATION;         // 計算用の周波数

static int16_t audio[AUDIO_TOTAL];
static int16_t audio_af[AUDIO_TOTAL_AF];
...
int main() {
  audio_features_init();
  printf("profile %d %lu\n", (int)AF_PROFILE, (unsigned long)AF_FRAME_HOP);

  SynthRng rng;
  synth_rng_seed(&rng, SYNTH_SEED_AUDIO);
  synth_audio(&rng, 0, AUDIO_TOTAL, audio);
  audio_stage_decimate(audio, AUDIO_TOTAL, audio_af);   // AF_PROFILE 0 では複写

  float out[AF_N_FEATURES];
  audio_features_full(audio_af, out);
  ...
  int k = 0;
  for (uint32_t s = 0; s + AF_IN_HOP_SAMPLES <= AUDIO_TOTAL; s += AF_IN_HOP_SAMPLES) {
