...
M1 ロギング・分岐点（期限 9/27）: #13 の実データでの実行と報告（docs/decisions/0014 の手順。学習側は収集日1+2、評価側は収集日3+4、`--eval-min 14`。走らせる前に `analysis/split.py data/raw --eval-min 14` の `eval` が収集日3+4の14本の名前と一致することを目で照合し直す）、判定は #14。#12 は収集日4で終わり、人が閉じた（9/24）。
