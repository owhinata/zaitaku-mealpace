...
- #12（収集日4）: 7セッションを録った（docs/log/2026-09-24.md に集計表）。人の決定で収集の最終日。`s` 34、`t`+`c`+`n` 54
  ...
  #12 の収集はこれで終わり。次は #13 の実データでの実行。
- ロボセンサー技研への問い合わせ未送付（代替センサ、docs/decisions/0002、#5）。

## 次にやること

M1 ロギング・分岐点（期限 9/27）: #13 の実データでの実行と報告（docs/decisions/0014 の手順。学習側は収集日1+2、評価側は収集日3+4、`--eval-min 14`。走らせる前に `analysis/split.py data/raw --eval-min 14` の `eval` が収集日3+4の14本の名前と一致することを目で照合し直す）、判定は #14。#12 の収集は収集日4で終わった（Issue を閉じるのは人）。
