...
**今（9/26）:** #25 は実装と実機の確認が済んだ。0018 の切り替え条件 (a)（装着すると基板の LED が見えない）に当たったので、外付け 2 灯への切り替えを人が決める
（決まれば別の Issue で `led_out.cpp` の書き込み先を替える。0018 への追記は人の決定の後）。案A / 案C も人が決める。#27 の `meal` の記録は切り替えが入ってから。
