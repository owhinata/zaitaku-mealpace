...
M2 検出器（9/28〜10/11）。M1 ロギング・分岐点の関門は 9/24 に通過（docs/decisions/0017。`gate-M1` タグは人が打つ）。
...
- #14: 関門のレビュー（Codex、`--base gate-M0`）を1回回した。指摘は1件（セッションのフォルダの中のファイルのリンクやコピーで、
  ...
  **人が M1 を通過とした（9/24、docs/decisions/0017）。** 条件として M2 の課題を3つ明記した: 日をまたいでも崩れないこと
  ...
  同時に満たす閾値は無い（事後の参考）。`gate-M1` タグと #14 を閉じるのは人。
...
M1 の後始末（人）: `gate-M1` タグを打つ、#14 を閉じる。次の関門（M2）のレビューは `--base gate-M1` で回す（タグが打たれてから）。
